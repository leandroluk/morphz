...
describe('template resolution', () => {
  ...
  it('resolves #entityName placeholder using labels', () => {
    const resolved = (Widget as unknown as { [STRUCT_META]: { fields: Record<string, { meta: { description?: string } }> } })[
      STRUCT_META
    ].fields.label.meta.description
  ...
  it('leaves unresolved placeholders untouched (no matching label)', () => {
    ...
    const resolved = (NoLabel as unknown as { [STRUCT_META]: { fields: Record<string, { meta: { description?: string } }> } })[
      STRUCT_META
    ].fields.label.meta.description
...
describe('pre hook', () => {
  class User extends Struct(
    { username: Text() },
    { pre: (val) => ({ ...(val as Record<string, unknown>), username: String((val as { username: unknown }).username).toLowerCase() }) },
  ) {}
...
describe('STRUCT_META', () => {
  ...
  it('rawObjectSchema is a bare ZodObject reflecting the field shape', () => {
    const meta = (Sample as unknown as { [STRUCT_META]: { rawObjectSchema: z.ZodObject; schema: z.ZodType } })[STRUCT_META]
  ...
  it('fields carries resolved descriptors', () => {
    const meta = (Sample as unknown as { [STRUCT_META]: { fields: Record<string, unknown> } })[STRUCT_META]
